...
- `screenshot.timers` - интервалы фотоотчёта после каждого запуска плейлиста в формате `HH:mm:ss` от `00:00:00` до `23:59:59`; пустой список отключает автоматический фотоотчёт.
