...
- **Синхронизация плейлиста** (`/api/menu/playlist/start-upload`): Перезапускает `play.video.service` после успешной синхронизации. Используйте этот endpoint, когда изменения могут повлиять на порядок или состав воспроизводимых файлов.

- **Синхронизация видео** (`/api/menu/video/start-upload`): **НЕ** перезапускает `play.video.service` после синхронизации. Используйте этот endpoint для обновления только видео-файлов без прерывания текущего воспроизведения.

- **Автоматическая синхронизация по расписанию**: Перезапускает `play.video.service` после успешной синхронизации, так как она может включать изменения плейлиста.

**Важно**: Оба типа синхронизации (плейлист и видео) загружают все файлы из манифеста backend. Различие заключается только в поведении перезапуска сервиса воспроизведения.
