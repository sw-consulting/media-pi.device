...
func TestHandleConfigurationUpdateClearsPhotoTimersWhenOmitted(t *testing.T) {
	ServerKey = "test-key"
	...
	if cfg := GetCurrentConfig(); len(cfg.Screenshot.Timers) != 0 {
		t.Fatalf("expected omitted screenshot timers to clear config, got %+v", cfg.Screenshot.Timers)
	}
}
