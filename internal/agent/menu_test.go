...
func TestHandleServiceStatusReturnsStatuses(t *testing.T) {
	...
	// PlaylistUploadServiceStatus and VideoUploadServiceStatus now report sync scheduler status
	// Since sync scheduler is not running in this test, both should be false
	if resp.Data.PlaylistUploadServiceStatus != false {
		t.Fatalf("expected playlist upload service (sync scheduler) to be inactive, got %v", resp.Data.PlaylistUploadServiceStatus)
	}
	if resp.Data.VideoUploadServiceStatus != false {
		t.Fatalf("expected video upload service (sync scheduler) to be inactive, got %v", resp.Data.VideoUploadServiceStatus)
	}
